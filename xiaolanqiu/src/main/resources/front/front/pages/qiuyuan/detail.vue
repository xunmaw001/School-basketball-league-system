			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.qiuyuanName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.qiuyuanContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.qiuyuanId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getqiuyuanCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        qiuyuanId: this.qiuyuanId,
                        yonghuId: this.user.yonghuId,
						qiuyuanLiuyanPingfenNumber: this.scoreData,
                        qiuyuanLiuyanText: this.commentData,
                    }
                    await this.$api.save(`qiuyuanLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('qiuyuan', this.qiuyuanId);
                    this.detail = res.data
					this.detail.qiuyuanContent = this.detail.qiuyuanContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.qiuyuanPhoto ? this.detail.qiuyuanPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getqiuyuanCollection() {
					let params = {
						page: 1,
						limit: 1,
						qiuyuanId: this.qiuyuanId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`qiuyuanCollection`, params);
					if (res.data.list.length == 1) {
						let qiuyuanCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('qiuyuanCollection', JSON.stringify([qiuyuanCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getqiuyuanCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('qiuyuanCollection', {
									yonghuId: _this.user.id,
									qiuyuanId: _this.detail.id,
									qiuyuanCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('qiuyuanLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						qiuyuanId: this.qiuyuanId
					});
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('qiuyuanId', _this.qiuyuanId);
                                // _this.$utils.jump('/pages/qiuyuanYuyue/add-or-update');

								await _this.$api.add('qiuyuanYuyue', {
									yonghuId: _this.user.id,
									qiuyuanId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/qiuyuanYuyue/list');
							}
						}
					});
				},
