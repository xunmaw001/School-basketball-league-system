	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.qiuyuanLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('qiuyuanLiuyan', this.qiuyuanLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.qiuyuanLiuyanPhoto ? this.detail.qiuyuanLiuyanPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('qiuyuanLiuyanId', _this.qiuyuanLiuyanId);
                                // _this.$utils.jump('/pages/qiuyuanLiuyanYuyue/add-or-update');

								await _this.$api.add('qiuyuanLiuyanYuyue', {
									yonghuId: _this.user.id,
									qiuyuanLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/qiuyuanLiuyanYuyue/list');
							}
						}
					});
				},
