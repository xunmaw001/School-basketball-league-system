	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.qiuyuanCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('qiuyuanCollection', this.qiuyuanCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.qiuyuanCollectionPhoto ? this.detail.qiuyuanCollectionPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('qiuyuanCollectionId', _this.qiuyuanCollectionId);
                                // _this.$utils.jump('/pages/qiuyuanCollectionYuyue/add-or-update');

								await _this.$api.add('qiuyuanCollectionYuyue', {
									yonghuId: _this.user.id,
									qiuyuanCollectionId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/qiuyuanCollectionYuyue/list');
							}
						}
					});
				},
