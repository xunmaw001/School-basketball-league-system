			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.qiuduiName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.qiuduiContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.qiuduiId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getqiuduiCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        qiuduiId: this.qiuduiId,
                        yonghuId: this.user.yonghuId,
						qiuduiLiuyanPingfenNumber: this.scoreData,
                        qiuduiLiuyanText: this.commentData,
                    }
                    await this.$api.save(`qiuduiLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('qiudui', this.qiuduiId);
                    this.detail = res.data
					this.detail.qiuduiContent = this.detail.qiuduiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.qiuduiPhoto ? this.detail.qiuduiPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getqiuduiCollection() {
					let params = {
						page: 1,
						limit: 1,
						qiuduiId: this.qiuduiId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`qiuduiCollection`, params);
					if (res.data.list.length == 1) {
						let qiuduiCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('qiuduiCollection', JSON.stringify([qiuduiCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getqiuduiCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('qiuduiCollection', {
									yonghuId: _this.user.id,
									qiuduiId: _this.detail.id,
									qiuduiCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('qiuduiLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						qiuduiId: this.qiuduiId
					});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('qiudui', _this.detail);
									_this.$utils.msg('取消成功');
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('qiuduiId', _this.qiuduiId);
                                // _this.$utils.jump('/pages/qiuduiYuyue/add-or-update');

								await _this.$api.add('qiuduiYuyue', {
									yonghuId: _this.user.id,
									qiuduiId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/qiuduiYuyue/list');
							}
						}
					});
				},
