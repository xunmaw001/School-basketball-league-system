				...
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.saishixinxiCollectionValue}}
							</view>
						</view>
					</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.saishixinxiCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('saishixinxiCollection', this.saishixinxiCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.saishixinxiCollectionPhoto ? this.detail.saishixinxiCollectionPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('saishixinxiCollectionId', _this.saishixinxiCollectionId);
                                // _this.$utils.jump('/pages/saishixinxiCollectionYuyue/add-or-update');

								await _this.$api.add('saishixinxiCollectionYuyue', {
									yonghuId: _this.user.id,
									saishixinxiCollectionId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/saishixinxiCollectionYuyue/list');
							}
						}
					});
				},
