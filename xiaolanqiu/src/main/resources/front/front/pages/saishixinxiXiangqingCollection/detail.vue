				...
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.saishixinxiXiangqingCollectionValue}}
							</view>
						</view>
					</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.saishixinxiXiangqingCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('saishixinxiXiangqingCollection', this.saishixinxiXiangqingCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.saishixinxiXiangqingCollectionPhoto ? this.detail.saishixinxiXiangqingCollectionPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('saishixinxiXiangqingCollectionId', _this.saishixinxiXiangqingCollectionId);
                                // _this.$utils.jump('/pages/saishixinxiXiangqingCollectionYuyue/add-or-update');

								await _this.$api.add('saishixinxiXiangqingCollectionYuyue', {
									yonghuId: _this.user.id,
									saishixinxiXiangqingCollectionId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/saishixinxiXiangqingCollectionYuyue/list');
							}
						}
					});
				},
