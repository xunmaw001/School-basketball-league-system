	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.qiuduiLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('qiuduiLiuyan', this.qiuduiLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.qiuduiLiuyanPhoto ? this.detail.qiuduiLiuyanPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('qiuduiLiuyanId', _this.qiuduiLiuyanId);
                                // _this.$utils.jump('/pages/qiuduiLiuyanYuyue/add-or-update');

								await _this.$api.add('qiuduiLiuyanYuyue', {
									yonghuId: _this.user.id,
									qiuduiLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/qiuduiLiuyanYuyue/list');
							}
						}
					});
				},
