	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.qiuduiLiuyanText" type="text" placeholder="留言内容"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('qiuduiLiuyan','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('qiuduiLiuyan','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
    ...
    <!--  回复模态框 -->
        ...
        <view class="list b-b" v-for="(item, index) in list" :key="index"
            ...
            <view class="wrapper" style="padding: 10rpx 3rpx;margin-left: 25upx;margin-top: 10upx;">
                <view style="font-size: 30rpx;display: flex;justify-content: space-between;">
                    ...
                    <view  style="float: right;padding: 10rpx 10rpx;" class="cuIcon-delete" @click.stop="onDeleteTap(item.id)"></view>
                        ...
                        <view class="address-box" style="padding: 10rpx;">
                            <text style="margin-left: 30rpx;">我的评价：{{item.qiuduiLiuyanText}}</text>
                        </view>
                ...
                <uni-icons v-show="isAuth('qiuduiLiuyan','回复')" style="float: right;" @tap="onReply(item.id)" type="chat-filled" size="20">
                </uni-icons>
	...
	export default {
		...
		methods: {
			...
			async submitReplyData(){
                ...
                await this.$api.update(`qiuduiLiuyan`, data);
                this.$refs.replyDeliver.close()
                this.mescroll.num = 1
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`qiuduiLiuyan`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('qiuduiLiuyan', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.qiuduiLiuyanShenheTime=shijian;
                            await _this.$api.update(`qiuduiLiuyan`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.qiuduiLiuyanText) {
                    searchForm['qiuduiLiuyanText'] = this.searchForm.qiuduiLiuyanText
                }

                let res = await this.$api.page(`qiuduiLiuyan`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
