	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.qiuduiCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('qiuduiCollection', this.qiuduiCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.qiuduiCollectionPhoto ? this.detail.qiuduiCollectionPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('qiuduiCollectionId', _this.qiuduiCollectionId);
                                // _this.$utils.jump('/pages/qiuduiCollectionYuyue/add-or-update');

								await _this.$api.add('qiuduiCollectionYuyue', {
									yonghuId: _this.user.id,
									qiuduiCollectionId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/qiuduiCollectionYuyue/list');
							}
						}
					});
				},
