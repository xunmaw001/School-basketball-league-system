	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.saishixinxiXiangqingName" type="text" placeholder="标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('saishixinxiXiangqing','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('saishixinxiXiangqing','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='saishixinxiXiangqingTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in saishixinxiXiangqingTypesList" :key="index" :class="saishixinxiXiangqingTypes===item.codeIndex?'tabActive':''"
                                  @tap="saishixinxiXiangqingTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
...
<view class="list-box5"
    ...
    <block v-for="(product,index) in list" :key="index">
        <view @tap="onDetailTap(product)" class="list-item"
            ...
            <view class="list-item-body" style="flex: 1;">
                <view
                        ...
                        class="title hide1">{{product.saishixinxiXiangqingName}}
                </view>
                <view v-if="product.saishixinxiXiangqingValue"
                      ...
                      class="title">{{product.saishixinxiXiangqingValue}}
                </view>
                <view style="display: flex;justify-content: space-between;padding: 0 10rpx;">
                    <text v-if="isAuth('saishixinxiXiangqing','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('saishixinxiXiangqing','删除')" class="cuIcon-delete"
...
saishixinxiId:null,
		...
		async onShow() {
			...
			let saishixinxiId = uni.getStorageSync("saishixinxiId");
			if(saishixinxiId != null && saishixinxiId !=''){
                ...
                //当前表的 类型 字段 字典表查询方法
                this.saishixinxiXiangqingTypesSelect()
		...
		methods: {
            ...
            //当前表的 类型 字段 字典表查询方法
            async saishixinxiXiangqingTypesSelect(){
                let saishixinxiXiangqingTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'saishixinxi_xiangqing_types',
                });
                saishixinxiXiangqingTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.saishixinxiXiangqingTypesList = saishixinxiXiangqingTypes.data.list
            },
            //当前表的 类型字段类别搜索
			saishixinxiXiangqingTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
				let params = {
					page: mescroll.num,
					limit: mescroll.size,
					saishixinxiId: this.saishixinxiId,
				}

                if (this.saishixinxiXiangqingTypes != 0) {
                    params.saishixinxiXiangqingTypes = this.saishixinxiXiangqingTypes
                }
                let _this = this
                let res = await this.$api.page(`saishixinxiXiangqing`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('saishixinxiXiangqing', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.saishixinxiXiangqingShenheTime=shijian;
                            await _this.$api.update(`saishixinxiXiangqing`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.saishixinxiXiangqingUuidNumber) {
                    searchForm['saishixinxiXiangqingUuidNumber'] = this.searchForm.saishixinxiXiangqingUuidNumber
                }
				if (this.searchForm.saishixinxiXiangqingName) {
                    searchForm['saishixinxiXiangqingName'] = this.searchForm.saishixinxiXiangqingName
                }

                let res = await this.$api.page(`saishixinxiXiangqing`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.saishixinxiXiangqingContent!=null){
                        item.saishixinxiXiangqingContent = item.saishixinxiXiangqingContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
