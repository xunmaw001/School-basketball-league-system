			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.saishixinxiXiangqingName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="xiangqing">
					<view class="shangpinxiangqing">
						...
						<view class="app">
							{{detail.saishixinxiXiangqingUuidNumber}}
						</view>
					</view>
				</view>
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.saishixinxiXiangqingValue}}
							</view>
						</view>
					</view>
				<view class="xiangqing" @tap="download(baseUrl + detail.saishixinxiXiangqingFile)">
					<view class="shangpinxiangqing">
						<view class="box">
							附件：
						</view>
						<view class="app">
							<view class="text" :style='{"color":"#333","textAlign":"center","fontSize":"28rpx"}'>下载</view>
						</view>
					</view>
				</view>
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.saishixinxiXiangqingDefen}}
							</view>
						</view>
					</view>
				<view class="xiangqing">
					<view class="shangpinxiangqing">
						...
						<view class="app">
							{{detail.saishixinxiXiangqingTime}}
						</view>
					</view>
				</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.saishixinxiXiangqingContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.saishixinxiXiangqingId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getsaishixinxiXiangqingCollection()
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('saishixinxiXiangqing', this.saishixinxiXiangqingId);
                    this.detail = res.data
					this.detail.saishixinxiXiangqingContent = this.detail.saishixinxiXiangqingContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.saishixinxiXiangqingPhoto ? this.detail.saishixinxiXiangqingPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getsaishixinxiXiangqingCollection() {
					let params = {
						page: 1,
						limit: 1,
						saishixinxiXiangqingId: this.saishixinxiXiangqingId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`saishixinxiXiangqingCollection`, params);
					if (res.data.list.length == 1) {
						let saishixinxiXiangqingCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('saishixinxiXiangqingCollection', JSON.stringify([saishixinxiXiangqingCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getsaishixinxiXiangqingCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('saishixinxiXiangqingCollection', {
									yonghuId: _this.user.id,
									saishixinxiXiangqingId: _this.detail.id,
									saishixinxiXiangqingCollectionTypes:1
								});
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('saishixinxiXiangqingId', _this.saishixinxiXiangqingId);
                                // _this.$utils.jump('/pages/saishixinxiXiangqingYuyue/add-or-update');

								await _this.$api.add('saishixinxiXiangqingYuyue', {
									yonghuId: _this.user.id,
									saishixinxiXiangqingId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/saishixinxiXiangqingYuyue/list');
							}
						}
					});
				},
