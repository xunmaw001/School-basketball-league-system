		...
		<view class="">
			...
			<view class="yinyuewai">
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.saishixinxiContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.saishixinxiId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getsaishixinxiCollection()
			...
			methods: {
				//返回
				async fanhui(){
				    uni.setStorageSync('saishixinxiId',this.detail.id);
				
					this.$utils.jump('../saishixinxiXiangqing/list');
					// this.$utils.tab("list")
				},
				//下载
				download(url) {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('saishixinxi', this.saishixinxiId);
                    this.detail = res.data
					this.detail.saishixinxiContent = this.detail.saishixinxiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.saishixinxiPhoto ? this.detail.saishixinxiPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getsaishixinxiCollection() {
					let params = {
						page: 1,
						limit: 1,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`saishixinxiCollection`, params);
					if (res.data.list.length == 1) {
						let saishixinxiCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('saishixinxiCollection', JSON.stringify([saishixinxiCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getsaishixinxiCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('saishixinxiCollection', {
									yonghuId: _this.user.id,
									saishixinxiId: _this.detail.id,
									saishixinxiCollectionTypes:1
								});
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('saishixinxiId', _this.saishixinxiId);
                                // _this.$utils.jump('/pages/saishixinxiYuyue/add-or-update');

								await _this.$api.add('saishixinxiYuyue', {
									yonghuId: _this.user.id,
									saishixinxiId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/saishixinxiYuyue/list');
							}
						}
					});
				},
